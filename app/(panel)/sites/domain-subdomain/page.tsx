...
type DomainOption = {
  id: string;
  domain: string;
  source: "available_domains" | "custom_domain";
};

type AvailableDomainRow = {
  id: string;
  domain: string | null;
  status: string | null;
...
export default function DomainSubdomainSitePage() {
  ...
  useEffect(() => {
    ...
    async function loadDomains() {
      ...
      const [{ data: availableData }, { data: siteDomainData }] = await Promise.all([
        supabase
      ...
      const byDomain = new Map<string, DomainOption>();

      for (const row of (availableData || []) as AvailableDomainRow[]) {
        const domain = cleanDomain(row.domain || "");
        if (!domain) continue;
        byDomain.set(domain, {
          id: `available:${row.id}`,
          domain,
          source: "available_domains",
        });
      }

      for (const row of (siteDomainData || []) as ConnectedSiteDomainRow[]) {
      ...
      const nextDomains = Array.from(byDomain.values()).sort((a, b) =>
        a.domain.localeCompare(b.domain)
      );

      setDomains(nextDomains);
      setSelectedDomain((current) =>
        current && nextDomains.some((domain) => domain.domain === current)
          ? current
          : nextDomains[0]?.domain || ""
      );
      setDomainsLoading(false);
    }
  ...
  async function handleCreate() {
    ...
    if (!selectedDomain) return setMsg("Selecione um dominio conectado.");
    ...
    try {
      ...
      if (!normalizeRes.ok || !normalizeJson?.ok) {
        setMsg(
          normalizeJson?.error ||
            "Site criado, mas nao foi possivel ajustar o dominio conectado."
        );
        return;
      }
            ...
            <div>
              ...
              >
                {domainsLoading ? (
                  ...
                  <option value="">Nenhum domínio conectado</option>
                ) : (
                  domains.map((domain) => (
              ...
              <p className="mt-2 text-xs font-semibold text-slate-500">
                Para os subdomínios funcionarem, deixe no DNS um registro A com nome * apontando para 179.197.64.7.
              </p>
            </div>
